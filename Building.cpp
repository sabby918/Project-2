...
void building::simulate() {
	...
	while (index < 10) {
		...
		int percentage = 50;
		requests = createCalls(requests, 50, 10);
		...
		myfile << endl;
		myfile << "RoundTime: "; timeCoversion(roundTime, myfile);
		myfile << endl << endl;
...
/*
void building::generate(int numfloors) {
	int current_floor = rand() % numfloors;
	int desired_floor = rand() % numfloors;
	while (current_floor == desired_floor) {
		desired_floor = rand() % numfloors;
	}
	call newCall;
	newCall.floor = current_floor;
	newCall.goal = desired_floor;
